...
export default function AIPhotoIdentifier({ item, onResult, onClose }) {
  const { apiKey, posItems } = useStore();
  ...
  const analyze = async (file) => {
    setError(''); setAnalyzing(true); setSuggestion(null);
    try {
      const b64 = await resizeImage(file);
      const content = [
        { type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: b64 } },
        { type: 'text', text: `This is a photo of a delivery item. The invoice name is: "${item.invoiceName || 'unknown'}".
...
{"productName":"exact product name visible on packaging or label","confidence":0-100}` },
      ...
      setSuggestion({ productName: parsed.productName, confidence: parsed.confidence, matches });
    } catch (err) {
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 200, display: 'flex', flexDirection: 'column', padding: 20, overflowY: 'auto' }}>
      <div style={{ background: 'var(--bg2)', borderRadius: 12, padding: 20, maxWidth: 480, width: '100%', margin: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>AI Identify Photo</h3>
          <button className="btn btn-ghost" style={{ padding: 4 }} onClick={onClose}><Icon name="x" size={18} /></button>
        </div>

        <div style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 12 }}>
          Item: <strong>{item.invoiceName}</strong>
        </div>
          ...
          <>
            ...
            {suggestion.matches.length > 0 ? (
              <>
                ...
                {suggestion.matches.slice(0, 5).map(r => (
              ...
              <div style={{ fontSize: 12, color: 'var(--text3)' }}>No POS matches found for "{suggestion.productName}"</div>
            )}
            <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
              <button className="btn btn-ghost" style={{ flex: 1, fontSize: 12 }}
                onClick={() => { setSuggestion(null); }}>Try Again</button>
